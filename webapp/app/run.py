...
# load model
model = joblib.load("../models/ML_pipeline_SVC.pkl")
...
def index():
    ...
    direct_category_counts = df[df.genre == 'direct'].drop(['id','message','original','genre'], axis = 1).sum()
    direct_category_names = list(direct_category_counts.index)

    social_category_counts = df[df.genre == 'social'].drop(['id','message','original','genre'], axis = 1).sum()
    social_category_names = list(social_category_counts.index)

    news_category_counts = df[df.genre == 'news'].drop(['id','message','original','genre'], axis = 1).sum()
    ...
    graphs = [
        ...
        },
        {
            ...
            'layout': {
                ...
                'xaxis': {
                    'title': "Category in direct genre"
                }
            }
        },
        {
            ...
            'layout': {
                ...
                'xaxis': {
                    'title': "Category in social genre"
                }
            }
        },
        {
